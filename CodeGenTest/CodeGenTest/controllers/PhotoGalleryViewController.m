...
- (void)loadView
{
    ...
    UIImage *image = [UIImage imageNamed:@"photo_gallery.png"];
    ...
    [button setTitle:@"launch photo viewer" forState:UIControlStateNormal];
    [button addTarget:self action:@selector(handleLaunchPhotoViewer:) forControlEvents:UIControlEventTouchUpInside];
    [view addSubview:button];

    button = [UIButton buttonWithType:UIButtonTypeRoundedRect];
    [button setFrame:CGRectMake(20, 75, 280, 20)];
    [button setTitle:@"launch screensaver" forState:UIControlStateNormal];
    [button addTarget:self action:@selector(handleLaunchScreensaver:) forControlEvents:UIControlEventTouchUpInside];
    [view addSubview:button];

    view.backgroundColor = [UIColor whiteColor];

    self.view = view;
    NSLog(@"PhotoGalleryViewController.loadView");
}
...
{
    [[HGAppDelegate getInstance] handleCommandWithData:kLaunchPhotoViewer data:nil];
}

- (IBAction)handleLaunchScreensaver:(id)sender
{
    [[HGAppDelegate getInstance] handleCommandWithData:kLaunchScreensaver data:nil];
}
